...
def generate_standalone_report(profile: Dict, output_dir: str = "reports", audit_results: Dict = None):
...
<body>
    ...
    # Audit section if provided
    if audit_results and audit_results.get("issues"):
        html += """
        <h2>Data Quality & Leakage Audit</h2>
        <table>
            <thead>
                <tr>
                    <th>Category</th>
        ...
        for issue in audit_results["issues"]:
            sev_class = "badge-danger" if issue["severity"] == "CRITICAL" else "badge-warning"
            html += f"""
                <tr>
                    <td><strong>{issue['category']}</strong></td>
                    <td><code>{issue['column']}</code></td>
                    <td><span class="stat-badge {sev_class}">{issue['severity']}</span></td>
                    <td>{issue['message']}</td>
                </tr>
            """
        html += """
